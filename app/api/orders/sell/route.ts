...
const logger = log("orders/sell");

export async function POST(req: NextRequest) {
  try {
    const limited = await enforce(limiters.money, `sell:${clientIp(req)}`);
    ...
    const trade = await executeTrade({ userId, side: "sell", symbol, name, shares, price });
    if (!trade.ok) return NextResponse.json({ error: trade.message }, { status: trade.status });

    return NextResponse.json(
      { message: "Sell order placed", orderId: trade.orderId, total: trade.total },
      { status: 201 },
    );
  } catch (err) {
    logger.error({ err }, "sell order failed");
